...
def _find_latest_pose_csv(base_path: Path) -> Optional[Path]:
    """
    Locate the most recently updated pose CSV generated by the Unity camera package.
    """
    patterns = [
        base_path / "poses" / "**" / "*_hmd_pose.csv",
        base_path / "**" / "*_poses" / "**" / "*_hmd_pose.csv",
        base_path / "**" / "*_hmd_pose.csv",
    ]
...
def _parse_pose_line(line: str) -> Optional[Dict[str, float]]:
    """
    Parse a single CSV pose line produced by CenterEyePoseLogger.
    Expected format: frameCount,timeUTC,posX,posY,posZ,rotX,rotY,rotZ,rotW,screenshotPath
    """
    parts = line.strip().split(",")
    if len(parts) < 5:
        return None
    try:
        return {"x": float(parts[2]), "y": float(parts[3]), "z": float(parts[4])}
    except ValueError:
        return None


def _extract_pose_from_lines(lines: list, timestamp: Optional[str]) -> Optional[Dict[str, float]]:
    """
    Extract the most recent pose from CSV lines, optionally filtering by timestamp substring.
    """
    for line in reversed(lines):
        if not line or line.startswith("frameCount"):
            continue
        if timestamp and timestamp not in line:
            continue
...
def _read_pose_from_csv(base_path: Path, timestamp: Optional[str]) -> Optional[Dict[str, float]]:
    """
    Read the latest pose record from the newest pose CSV, preferring entries that match timestamp.
    """
    pose_file = _find_latest_pose_csv(base_path)
    ...
    try:
        ...
        return None
    return _extract_pose_from_lines(lines, timestamp)
...
class Unity3DPerception(PerceptionInterface):
    ...
    def __init__(
        ...
        self.agent_steps: Dict[str, int] = {}
        self.latest_pose: Dict[str, Dict[str, float]] = {}
        self.initial_pose: Dict[str, Dict[str, float]] = {}
        self.boundary_warned: Dict[str, bool] = {}
        self.boundary_state: Dict[str, Dict[str, Any]] = {}
        self._last_request_label: Dict[str, str] = {}
    ...
    def get_visible_objects(self, agent_id: str, position: Any) -> List[str]:
        ...
        self._last_request_label[agent_id] = timestamp
        ...
        if screenshot_path:
            ...
            self._update_pose_cache(agent_id, timestamp)
    ...
    def execute_action(self, agent_id: str, action: str, params: Optional[Dict] = None) -> Dict[str, Any]:
        ...
        return {
            "position": self.latest_pose.get(agent_id, self.agent_steps[agent_id]),
            "world_position": self.latest_pose.get(agent_id),
            "rotation": None,
            "velocity": None,
            "visible_objects": [],  # Will be updated in next PerceptionNode
            "movement_skipped": False,
            "boundary_info": self.boundary_state.get(agent_id),
        }
    ...
    def _update_pose_cache(self, agent_id: str, timestamp: Optional[str] = None) -> Optional[Dict[str, float]]:
        pose = _read_pose_from_csv(self.unity_output_base_path, timestamp)
        if pose is None:
            return None
        self.latest_pose[agent_id] = pose
        if agent_id not in self.initial_pose:
            self.initial_pose[agent_id] = pose
        self._check_and_warn_boundary(agent_id)
        return pose

    def _check_and_warn_boundary(self, agent_id: str) -> bool:
        init = self.initial_pose.get(agent_id)
        current = self.latest_pose.get(agent_id)
        if not init or not current:
            return False

        deltas = {axis: current[axis] - init[axis] for axis in ("x", "y", "z")}
        hit_axes = [axis for axis, val in deltas.items() if abs(val) >= BOUNDARY_LIMIT]
        out_of_bounds = len(hit_axes) > 0
        self.boundary_state[agent_id] = {
            "out_of_bounds": out_of_bounds,
            "hit_axes": hit_axes,
            "deltas": deltas,
            "limit": BOUNDARY_LIMIT,
        }
        if out_of_bounds and not self.boundary_warned.get(agent_id):
            print(
                f"[Unity3DPerception] WARNING: Agent '{agent_id}' reached movement boundary {BOUNDARY_LIMIT} "
                f"(Δx={deltas['x']:.2f}, Δy={deltas['y']:.2f}, Δz={deltas['z']:.2f})"
            )
            self.boundary_warned[agent_id] = True
        elif not out_of_bounds:
            # Reset flag if agent moves back within bounds (for future alerts)
            self.boundary_warned[agent_id] = False
        return out_of_bounds

    def _is_out_of_bounds(self, agent_id: str) -> bool:
        if agent_id not in self.initial_pose or agent_id not in self.latest_pose:
            return False
        return self._check_and_warn_boundary(agent_id)

    def get_latest_world_pose(self, agent_id: str) -> Optional[Dict[str, float]]:
        if agent_id in self.latest_pose:
            return self.latest_pose[agent_id]
        return self._update_pose_cache(agent_id, self._last_request_label.get(agent_id))
...
class UnityCameraPerception(PerceptionInterface):
    ...
    def _check_and_warn_boundary(self, agent_id: str) -> bool:
        ...
        if out_of_bounds and not self.boundary_warned.get(agent_id):
            print(
                f"[UnityCameraPerception] WARNING: Agent '{agent_id}' reached movement boundary {BOUNDARY_LIMIT} "
                f"(Δx={deltas['x']:.2f}, Δy={deltas['y']:.2f}, Δz={deltas['z']:.2f})"
            )
            self.boundary_warned[agent_id] = True
        elif not out_of_bounds:
