...
def read_camera_position_from_poses(
    ...
    """
    Read camera position (x, y, z) from the poses CSV file based on screenshot filename.
    
    The poses CSV is generated by Unity's CenterEyePoseLogger and contains:
    frameCount, timeUtc, x, y, z, qx, qy, qz, qw, screenshotName
    
    Args:
        screenshot_path: Full path to the screenshot file
        unity_output_base_path: Base path where Unity saves outputs (e.g., "E:/output")
                               If None, will try to infer from screenshot path or config
    ...
    if not unity_output_base_path:
        # Try to infer from screenshot path (assuming structure: {base}/screenshots/{camera}/...)
        try:
    ...
    if not unity_output_base_path:
        print(f"[read_camera_position] Cannot determine unity_output_base_path")
        return None
    
    # Search for pose CSV files in poses directory
    # Structure: {unity_output_base_path}/poses/{CameraName}/*.csv
        ...
        try:
            with open(csv_file, 'r', encoding='utf-8') as f:
                for line in f:
                    ...
                    # Check if this line contains our screenshot name
                    if screenshot_name in line:
                        # Parse CSV: frameCount, timeUtc, x, y, z, qx, qy, qz, qw, screenshotName
                        parts = line.split(',')
                        if len(parts) >= 5:
                            try:
                                x = float(parts[2])
                                y = float(parts[3])
                                z = float(parts[4])
                                print(f"[read_camera_position] Found position for {screenshot_name}: ({x:.2f}, {y:.2f}, {z:.2f})")
                                return (x, y, z)
                            except (ValueError, IndexError) as e:
                                print(f"[read_camera_position] Error parsing position from line: {e}")
                                continue
        except Exception as e:
            print(f"[read_camera_position] Error reading {csv_file}: {e}")
            continue
    
    print(f"[read_camera_position] Position not found for screenshot: {screenshot_name}")
    return None
